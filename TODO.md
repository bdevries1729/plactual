...
- don't let another item be created when it already exists! The item is created immediately upon finishing the Plaid link UI, so this needs to happen onEvent callback for the eventName equal to SELECT_INSTITUTION. In that case close link and show the user an error message.
